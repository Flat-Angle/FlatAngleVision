...
def data_loader(FLAGS):
    with tf.device('/cpu:0'):
        ...
        image_list_LR = [_ for _ in image_list_LR if _.endswith('.png') or _.endswith('.jpg')]
...
def test_data_loader(FLAGS):
    ...
    image_list_LR = [os.path.join(FLAGS.input_dir_LR, _) for _ in image_list_LR_temp if _.split('.')[-1] == 'png' or 'jpg']
    ...
    def preprocess_test(name, mode):
        im = imageio.imread(name, pilmode="RGB").astype(np.float32)
